...
import { avaiableAnytimeTranslations } from './translations';

@Component({
  selector: 'app-available-anytime-moving-services',
  standalone: true,
  imports: [],
  templateUrl: './available-anytime-moving-services.component.html'
})
export class AvailableAnytimeMovingServicesComponent implements OnInit, OnDestroy {
  ...
  getTranslation(key: string): string {
    return avaiableAnytimeTranslations[this.currentLanguage]?.[key] || avaiableAnytimeTranslations['en']?.[key];
  }
  ...
  setInitialPageConfiguration(): void {
    ...
    const canonicalUrl = `https://www.floand-go.com/${this.currentLanguage}/available-anytime-moving-services`;
    ...
    this.updateMetaTag('keywords', 'mudanças lisboa, mudanças 24h, empresa de mudanças, Flo and Go');
    ...
    this.updateMetaProperty('og:image:alt', 'Logo da empresa Flo and Go - mudanças em Lisboa 24h');
  ...
  setHreflangTags(): void {
    if (isPlatformBrowser(this.platformId) || isPlatformServer(this.platformId)) {
      const pageSlug = 'available-anytime-moving-services';
