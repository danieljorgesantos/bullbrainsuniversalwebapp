...
import { lastMinuteChangesTranslations } from './translations';

@Component({
  selector: 'app-last-minute-changes',
  standalone: true,
  imports: [],
  templateUrl: './last-minute-changes.component.html'
})
export class LastMinuteChangesComponent implements OnInit, OnDestroy {
  ...
  getTranslation(key: string): string {
    return lastMinuteChangesTranslations[this.currentLanguage]?.[key] || lastMinuteChangesTranslations['en']?.[key];
  }
  ...
  setInitialPageConfiguration(): void {
    ...
    const canonicalUrl = `https://www.floand-go.com/${this.currentLanguage}/last-minute-changes`;
    ...
    this.updateMetaTag('keywords', 'mudanças urgentes, mudanças última hora, Flo and Go');
    ...
    this.updateMetaProperty('og:image:alt', 'Logo da empresa Flo and Go - mudanças urgentes');
  ...
  setHreflangTags(): void {
    if (isPlatformBrowser(this.platformId) || isPlatformServer(this.platformId)) {
      const pageSlug = 'last-minute-changes';
