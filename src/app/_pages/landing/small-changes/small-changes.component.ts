...
import { smallChangesTranslations } from './translations';

@Component({
  selector: 'app-small-changes',
  standalone: true,
  imports: [],
  templateUrl: './small-changes.component.html'
})
export class SmallChangesComponent {
 // Language
  ...
  getTranslation(key: string) {
    return smallChangesTranslations[this.currentLanguage]?.[key] || smallChangesTranslations['en']?.[key];
  }

  setInitialPageConfiguration() {
    const titleToSet = smallChangesTranslations[this.currentLanguage]?.meta_title || smallChangesTranslations['en']?.meta_title;
    const descriptionToSet = smallChangesTranslations[this.currentLanguage]?.meta_description || smallChangesTranslations['en']?.meta_description;
